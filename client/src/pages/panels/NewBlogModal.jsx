...
export default function NewBlogModal({
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      if (event.key === "Escape") {
        onClose(); // Triggers the close function when Escape is pressed
      }
    };

    // Attach listener to the whole document window
    window.addEventListener("keydown", handleKeyDown);

    // Clean up the listener when the modal closes/unmounts
    return () => {
  ...
  return (
    <div id="new-blog-modal-panel" className="modal-overlay">
