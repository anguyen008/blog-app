...
export default function UserSettingsPanel() {
  ...
  function setp(p) {
    return (e) => {
      setPasswords((f) => ({ ...f, [p]: e.target.value }));
      setError("");
    };
  }
  ...
  async function savePassword(e) {
    ...
    if (passwords.next.length < 6) {
      setError("New password must be 6+ characters.");
      return;
    }
    ...
    if (passwords.next === passwords.current) {
      setError("Current and New passwords can't be the same");
      return;
    }
    setSaving(true);
    try {
  ...
  async function deleteAccount() {
    if (typedName.trim().toLowerCase() !== user.name.trim().toLowerCase())
      return;
    setSaving(true);
    try {
      ...
      <div className="settings-page fade-up">
        ...
        {tab === "profile" && (
          <>
            ...
            <div className="settings-section danger-zone fade-up">
              <h3>Danger zone</h3>
              <p className="danger-text">
                Permanently deletes your account, all blogs, and every post.
                Cannot be undone.
              </p>
              {!deleteConfirm ? (
                <button
